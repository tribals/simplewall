...
)
{
	...
	if (!is_providerexist || !is_sublayerexist)
	{
		...
		if (!is_sublayerexist)
		{
			...
			sublayer.weight = (UINT16)_r_config_getlong_ex (L"SublayerWeight", FW_SUBLAYER_WEIGHT, NULL);
	...
	{
		if (_r_config_getboolean_ex (L"IsPacketQueuingEnabled", TRUE, NULL))
		{
...
)
{
	...
	if (_r_config_getboolean_ex (L"UseStealthMode", TRUE, NULL) || _r_config_getboolean_ex (L"BlockInboundConnections", TRUE, NULL))
	{

...
)
{
	...
	if (_r_wnd_sendmessage (hwnd, 0, WM_NOTIFICATION, 0, (LPARAM)ptr_app->notification))
	{
		...
		{
			if (!_r_config_getboolean_ex (L"IsNotificationsFullscreenSilentMode", TRUE, NULL) || !_r_wnd_isfullscreenmode ())
...
)
{
	...
	is_intray = _r_config_getboolean_ex (L"IsNotificationsOnTray", FALSE, NULL);
